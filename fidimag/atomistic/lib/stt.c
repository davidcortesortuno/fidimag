...
#pragma omp parallel for
  for (int index = 0; index < n; index++) {
    ...
    double c = 6 * sqrt(dm_dt[i] * dm_dt[i] + dm_dt[j] * dm_dt[j] +
                        dm_dt[k] * dm_dt[k]);
...
#pragma omp parallel for
  for (int index = 0; index < n; index++) {
    ...
    double alpha2 = 1 / (1 + alpha[index] * alpha[index]);
    double coeff = -gamma * alpha2;
    ...
    // the above part is standard LLG equation.
    double tx = cross_x(m[i], m[j], m[k], dm[i], dm[j], dm[k]) / tau_sd;
    double ty = cross_y(m[i], m[j], m[k], dm[i], dm[j], dm[k]) / tau_sd;
    double tz = cross_z(m[i], m[j], m[k], dm[i], dm[j], dm[k]) / tau_sd;

    dm_dt[i] += alpha2 * (-tx + alpha[index] * dm[i] / tau_sd);
    dm_dt[j] += alpha2 * (-ty + alpha[index] * dm[j] / tau_sd);
    dm_dt[k] += alpha2 * (-tz + alpha[index] * dm[k] / tau_sd);
    ...
    //=================================================
    dm_dt[i + 3 * n] = D * h_lap[i] + tx - dm[i] / tau_sf + u0 * h_stt[i];
    dm_dt[j + 3 * n] = D * h_lap[j] + ty - dm[j] / tau_sf + u0 * h_stt[j];
    dm_dt[k + 3 * n] = D * h_lap[k] + tz - dm[k] / tau_sf + u0 * h_stt[k];
  }
}
